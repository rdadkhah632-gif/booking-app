...
type Props = {
  ...
  requestBooking: (request: BookingRequest) => RequestBooking | undefined | null
            ...
            <div className="customer-notification-card-row">
              <div style={{ flex: 1, minWidth: 260 }}>
                ...
                >
                  ...
                  >
                    <p className="small muted">{t('notifications.currentConfirmedAppointment', 'Current confirmed appointment')}</p>
                    <strong>
                  ...
                  >
                    <p className="small muted">{t('notifications.requestedNewAppointment', 'Requested new appointment')}</p>
                    <strong>{new Date(request.requested_start_at).toLocaleString()}</strong>
                  </div>
                </div>

                <p className="small muted" style={{ marginTop: '0.75rem' }}>
                  {t('myBookings.card.requestedStaff')}: {requestedStaffName(request)}
                </p>

                <p className="small muted">
                  {t('myBookings.card.requestedDuration')}: {request.requested_duration_minutes} minutes
                </p>

                <p className="small muted" style={{ marginTop: '0.5rem' }}>
                  {t('notifications.originalRemainsConfirmed', 'Your original booking remains confirmed until the business accepts this request.')}
                </p>
              </div>

              <div className="customer-notification-card-actions">
                <Link href="/my-bookings" className="btn btn-accent">
                  {t('notifications.viewBooking', 'View booking')}
                </Link>
              </div>
            </div>
          </div>
