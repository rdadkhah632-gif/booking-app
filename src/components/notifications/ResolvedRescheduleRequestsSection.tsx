...
export default function ResolvedRescheduleRequestsSection({
  ...
  return (
    <div className="customer-notification-section">
      <div>
        <p className="small muted">{t('notifications.requestHistory', 'Request history')}</p>
          ...
          >
            <div className="customer-notification-card-row">
              <div style={{ flex: 1, minWidth: 260 }}>
                ...
                <p className="small muted">
                  {t('notifications.requestedTime', 'Requested time')}: {new Date(request.requested_start_at).toLocaleString()}
                </p>
                ...
                {request.response_message && (
                  <p className="small muted" style={{ marginTop: '0.5rem' }}>
                    {t('notifications.businessResponse', 'Business response')}: {request.response_message}
                  </p>
                )}

                <p className="small muted" style={{ marginTop: '0.5rem' }}>
                  {t('notifications.updated', 'Updated')}: {request.updated_at
