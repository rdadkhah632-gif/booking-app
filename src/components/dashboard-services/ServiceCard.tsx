...
type Props = {
  ...
  updateLocalService: (id: string, field: keyof Service, value: string | number | boolean) => void
          ...
          <div className="service-card-top-row">
            <div style={{ flex: 1, minWidth: 260 }}>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '0.55rem' }}>
                <strong>{service.name || t('dashboardServices.card.untitled', 'Untitled service')}</strong>

                <ServiceStatusBadge
                  label={service.active ? t('dashboardServices.card.visible', 'Visible') : t('dashboardServices.card.hidden', 'Hidden')}
                  tone={service.active ? 'success' : 'warning'}
                />

                <ServiceStatusBadge
                  label={isBookable ? t('dashboardServices.card.bookable', 'Bookable') : t('dashboardServices.card.notBookable', 'Not bookable yet')}
                ...
                {assignedStaff.length > 0 ? (
                  ...
                  <ServiceStatusBadge
                    label={t('dashboardServices.card.noStaffAssigned', 'No staff assigned')}
                    tone="warning"
                  />
                )}
              </div>
              ...
              {!isEditing && (
                <>
                  <p className="small muted">
                    {service.duration_minutes} {t('common.minutes', 'minutes')} · £{Number(service.price).toFixed(2)}
                  </p>
                  ...
                  {service.description ? (
                    ...
                    <p className="small muted" style={{ marginTop: '0.45rem' }}>
                      {t('dashboardServices.card.noDescription', 'No description added yet.')}
                    </p>
                  )}

                  <div className="card" style={{ background: 'var(--surface-2)', padding: '0.85rem', marginTop: '0.85rem' }}>
                    <p className="small muted">{t('dashboardServices.card.bookability', 'Bookability')}</p>

                    <strong>
                      {isBookable
                        ? t('dashboardServices.card.customersCanBook', 'Customers can book this service')
                        : t('dashboardServices.card.completeSetup', 'Complete setup before customers can book this service')}
                    </strong>
                    ...
                    <p className="small muted" style={{ marginTop: '0.35rem' }}>
                      {t('support.business.staff', 'Staff')}: {assignedStaff.length > 0
                        ? assignedStaff.map((staff) => `${staff.name}${staff.role_title ? ` — ${staff.role_title}` : ''}`).join(', ')
                        : t('dashboardServices.card.assignStaffHint', 'Assign active staff to make this service bookable.')}
                    </p>
                  </div>
                </>
              )}
              ...
              {isEditing && (
                <div style={{ display: 'grid', gap: '0.75rem', marginTop: '0.75rem' }}>
                  <input
                    placeholder={t('dashboardServices.card.serviceNamePlaceholder', 'Service name')}
                    value={service.name || ''}
                  ...
                  <div className="service-edit-grid">
                    <input
                      type="number"
                      placeholder={t('dashboardServices.create.durationPlaceholder', 'Duration')}
                  ...
                  <textarea
                    placeholder={t('dashboardServices.card.descriptionPlaceholder', 'Service description optional')}
                  ...
                  <label className="card" style={{ background: 'var(--surface-2)', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center' }}>
                      ...
                      <div>
                        <strong>{t('dashboardServices.card.visibleToCustomers', 'Visible to customers')}</strong>
                        <p className="small muted">
                          {t('dashboardServices.card.hiddenHint', 'Hidden services stay saved but will not be offered for booking.')}
                        </p>
                      </div>
                    </div>
                  </label>
            ...
            <div className="service-card-actions">
              {isEditing ? (
                ...
                <>
                  ...
                  {service.image_url ? (
                    <ServiceStatusBadge label={t('dashboardServices.card.imageAdded', 'Image added')} tone="muted" />
                  ) : (
                    <ServiceStatusBadge label={t('dashboardServices.card.noImage', 'No image')} tone="muted" />
                  )}
                </>
              )}
            </div>
