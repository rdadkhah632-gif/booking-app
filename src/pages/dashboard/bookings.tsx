...
export default function Bookings() {
  ...
  async function getBusinessContext(sessionUserId: string) {
    ...
    if (businessId && !Array.isArray(businessId)) {
      ...
      if (!selected) {
        throw new Error(t('dashboardBookings.error.noAccess', 'You do not have access to this business.'))
      }
      return selected
    }
  ...
  async function loadBookings(options?: { keepSuccess?: boolean; silent?: boolean }) {
    ...
    try {
      ...
      setError(err.message || t('dashboardBookings.error.load', 'Could not load bookings.'))
      setPageLoading(false)
    }
  }
...
let refreshTimer: number | null = null
  ...
  function serviceName(booking: Booking) {
    return booking.services?.name || t('dashboardBookings.notification.defaultService', 'your appointment')
  }
  ...
  async function cancelBooking(booking: Booking) {
    const confirmed = confirm(t('dashboardBookings.confirm.cancel', 'Cancel this booking? This will also show as cancelled to the customer.'))
    if (!confirmed) return
    ...
    await createCustomerNotification({
      ...
      title: t('dashboardBookings.notification.cancelledTitle', 'Booking cancelled'),
      message: t('dashboardBookings.notification.cancelledMessage', 'Your booking was cancelled by the business.'),
      actionUrl: '/my-bookings'
    })
  ...
  async function completeBooking(booking: Booking) {
    const confirmed = confirm(t('dashboardBookings.confirm.complete', 'Mark this appointment as completed?'))
    if (!confirmed) return
    ...
    await createCustomerNotification({
      ...
      title: t('dashboardBookings.notification.completedTitle', 'Appointment completed'),
      message: t('dashboardBookings.notification.completedMessage', 'Your appointment has been marked as completed.'),
      actionUrl: '/my-bookings'
    })
  ...
  function dateRangeForFilter(filter: RangeFilter) {
    ...
    if (filter === 'today') {
      return { start: today, end: endOfDay(today), label: t('dashboardHome.summary.today', 'Today') }
    }

    if (filter === 'tomorrow') {
      const tomorrow = addDays(today, 1)
      return { start: tomorrow, end: endOfDay(tomorrow), label: t('dashboardBookings.range.tomorrow', 'Tomorrow') }
    }

    if (filter === 'week') {
      return { start: today, end: endOfDay(addDays(today, 6)), label: t('dashboardHome.schedule.title', 'Next 7 days') }
    }
    ...
    return { start: null, end: null, label: filter === 'history' ? t('dashboardBookings.summary.history', 'History') : t('dashboardBookings.range.upcoming', 'All upcoming') }
  }
  ...
  const activeFilterSummary = [
    selectedRange.label,
    statusFilter === 'all' ? t('dashboardBookings.status.all', 'All statuses') : statusLabel(statusFilter),
    searchTerm.trim() ? `${t('dashboardBookings.filters.searchShort', 'Search')}: ${searchTerm.trim()}` : ''
  ].filter(Boolean).join(' · ')
  ...
  function setSummaryView(filter: RangeFilter, nextStatus?: string) {
    ...
    replaceBookingsQuery({ nextFilter: filter, nextStatus: nextStatus || statusFilter })
  }
  ...
  return (
    ...
    >
      ...
      {success && (
        ...
        >
          <div className="booking-success-row">
            <div>
              <p className="small" style={{ color: 'var(--success)' }}>{t('dashboardBookings.success.actionCompleted', 'Action completed')}</p>
      ...
      {pageLoading && (
        <div className="card">
          <p className="muted">{t('dashboardBookings.loading', 'Loading bookings...')}</p>
        </div>
      )}
