...
export default function SupportMessagesPage() {
  ...
  async function loadMessages() {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, subject, category, message, status, priority, created_at, updated_at",
      )
  ...
  const openMessages = useMemo(() => {
    return messages.filter(
      (message) =>
        !["resolved", "closed"].includes(String(message.status || "open")),
    );
  }, [messages]);
          ...
          <div className="support-messages-header">
            <div>
              <p className="small" style={{ color: "var(--accent)" }}>
                {t("support.messages.kicker", "Support conversations")}
              </p>
              <h1 className="page-title">
                {t("support.messages.title", "My support messages")}
              </h1>
            ...
            <div className="support-messages-actions">
              ...
              <Link href={workspaceHref} className="btn btn-ghost">
                {workspaceLabel}
              </Link>
              <button
                type="button"
              ...
              <div className="grid-3">
                <div className="card">
                  <p className="small muted">
                    {t(
                      "support.messages.openConversations",
                      "Open conversations",
                    )}
                  </p>
                  ...
                  <p className="small muted">
                    {t(
                      "support.messages.openConversationsBody",
                      "Waiting, open or in review",
                    )}
                  </p>
                </div>
                <div className="card">
                  <p className="small muted">
                    {t(
                      "support.messages.totalConversations",
                      "Total conversations",
                    )}
                  </p>
                ...
                <div className="card support-help-card">
                  ...
                  <h2>
                    {t(
                      "support.messages.startFromSupport",
                      "Start from Support",
                    )}
                  </h2>
                  <p className="small muted">
                ...
                <div className="support-message-list">
                  {messages.map((message) => (
                    <Link
                      ...
                      className="card support-message-card"
                    >
                      <div>
                        <div className="support-message-title-row">
                          <strong>
                            {message.subject ||
                              message.category ||
                              t(
                                "support.messages.requestFallback",
                                "Support request",
                              )}
                          </strong>
