...
const setupIssueMessages = useMemo(() => {
  ...
  const selectedSlotForComponents = selectedDate && selectedTime
    ? {
        startAt: `${selectedDate}T${selectedTime}:00`,
        label: selectedTime,
        staffMemberId: selectedStaffChoice === 'any' ? null : selectedStaffChoice
      }
    : null
       ...
       <div className="booking-page-grid">
          <section>
            <PublicBusinessServices
              services={visibleServices.filter((item) => item.isBookable).map((item) => item.service)}
