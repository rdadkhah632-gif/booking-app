...
export default function AdminIndexPage() {
  ...
  const [countsByBusiness, setCountsByBusiness] = useState<
    Record<string, BusinessCounts>
  >({});
  const [billingByBusiness, setBillingByBusiness] = useState<
    Record<string, BillingState>
  >({});
  const [notificationSummary, setNotificationSummary] =
    useState<NotificationSummary>({ unread: 0, adminSent: 0 });
  ...
  function needsAttention(business: BusinessRow) {
    ...
    return (
      ...
      (billing.billing_status === "free_trial" &&
        trialDays !== null &&
        trialDays <= 7)
    );
  }
  ...
  const summary = useMemo(() => {
    ...
    const ready = businesses.filter(
      (business) => readinessIssues(business).length === 0,
    ).length;
    const trial = businesses.filter((business) =>
      ["free_trial", "founding_free"].includes(
        getBillingState(business.id).billing_status,
      ),
    ).length;
    const active = businesses.filter((business) =>
      ["active", "manual_comped"].includes(
        getBillingState(business.id).billing_status,
      ),
    ).length;
    const paymentAttention = businesses.filter((business) =>
      ["past_due", "cancelled", "paused"].includes(
        getBillingState(business.id).billing_status,
      ),
    ).length;
    const notConfigured = businesses.filter(
      (business) =>
        getBillingState(business.id).billing_status === "not_configured",
    ).length;
    const attention = businesses.filter((business) =>
      needsAttention(business),
    ).length;
    ...
    return {
      ...
      businessRoleUsers: users.filter((user) => user.role === "business")
        .length,
  ...
  async function loadCounts(businessIds: string[]) {
    ...
    const staffBusinessMap = (staffData || []).reduce(
      (map: Record<string, string>, row: any) => {
  ...
  async function loadSupportSummary() {
    ...
    setSupportSummary({
      ...
      waiting: rows.filter((row: any) =>
        ["waiting", "waiting_for_user", "in_review"].includes(
          String(row.status || ""),
        ),
      ).length,
    ...
    <main>
      ...
      >
        <div className="admin-shell">
          <div className="admin-header">
            <div>
              ...
              <p className="page-sub" style={{ marginTop: "0.5rem" }}>
                Control onboarding, trials, subscriptions, users, platform
                messaging and launch readiness from one admin workspace.
              </p>
            </div>
          ...
          <details className="admin-mobile-overview">
            <summary>
              {t(
                "admin.operations.mobileOverview",
                "Overview and operator tools",
              )}
            </summary>
            <div className="admin-mobile-metrics">
              <span>
                <strong>{summary.attention}</strong>
                {t("admin.operations.needsAttention", "Needs attention")}
              </span>
              <span>
                <strong>{supportSummary.open}</strong>
                {t("admin.operations.openSupport", "Open support requests")}
              </span>
              <span>
                <strong>{summary.businesses}</strong>
                {t("admin.operations.businesses", "Businesses")}
              </span>
              <span>
            ...
            <div className="admin-mobile-tools">
              ...
              <Link href="/admin/notifications">
                {t("admin.operations.notifications", "Notifications")}
              </Link>
              <Link href="/admin/support">
                {t("admin.operations.support", "Support")}
              </Link>
            </div>
          </details>

          <div className="grid-4 admin-desktop-overview">
            <div className="card admin-metric-card">
              ...
              <p className="small muted">
                {summary.published}{" "}
                {t("admin.operations.published", "published")} · {summary.draft}{" "}
                {t("admin.operations.draft", "draft")}
              </p>
            </div>
            ...
            <div className="card admin-metric-card">
              <p className="small muted">
                {t("admin.operations.readyBusinesses", "Booking ready")}
              </p>
              <h2>{summary.ready}</h2>
              <p className="small muted">
                {summary.businesses - summary.ready}{" "}
                {t("admin.operations.notReady", "not ready")}
              </p>
            </div>
            ...
            <div className="card admin-metric-card">
              <p className="small muted">
                {t(
                  "admin.operations.billingActiveTrial",
                  "Billing active / trial",
                )}
              </p>
              <h2>
              ...
              <p className="small muted">
                ...
                {t("admin.operations.paymentAttention", "payment attention")} ·{" "}
                {summary.notConfigured}{" "}
                {t("admin.operations.notConfigured", "not configured")}
              </p>
            </div>
          </div>
          ...
          <div className="grid-4 admin-desktop-overview">
            <div className="card admin-metric-card">
              ...
              <h2>{summary.users}</h2>
              <p className="small muted">
                {summary.admins} admin account{summary.admins === 1 ? "" : "s"}
              </p>
            </div>
            ...
            <div className="card admin-metric-card">
              <p className="small muted">
                {t(
                  "admin.operations.activeMonthlyValue",
            ...
            <Link
              href="/admin/support"
              className="card admin-metric-card admin-metric-link-card"
            >
              <p className="small muted">
                {t("admin.operations.openSupport", "Open support requests")}
              </p>
              ...
              <p className="small muted">
                {supportSummary.waiting}{" "}
                {t("admin.operations.waitingSupport", "waiting")} ·{" "}
                {supportSummary.total}{" "}
                {t("admin.operations.totalSupport", "total")}
              </p>
            </Link>
          </div>

          <div className="card admin-operational-note admin-desktop-overview">
            <div>
              <p className="small muted">
                {t("admin.operations.dataSources", "Operational data sources")}
              </p>
              <h2>
                {t(
                  "admin.operations.readOnlyTitle",
                  "Read-only launch visibility",
                )}
              </h2>
              ...
              <p className="small muted" style={{ marginTop: "0.35rem" }}>
                ...
                {t(
                  "admin.operations.unreadNotifications",
                  "unread notifications",
                )}{" "}
                · {notificationSummary.adminSent}{" "}
                {t(
                  "admin.operations.operatorNotices",
                  "operator notices loaded",
                )}
              </p>
            </div>
            <div className="admin-actions">
          ...
          <div className="grid-3 admin-desktop-overview">
            <Link href="/admin/businesses" className="card admin-control-card">
              ...
              <p className="small muted" style={{ marginTop: "0.4rem" }}>
                Manage publishing, custom prices, subscription state, trial
                periods and business readiness.
              </p>
            </Link>
            ...
            <Link
              ...
              className="card admin-control-card"
            >
              <p className="small muted">Platform messaging</p>
              <h3>Notifications and promotions</h3>
              <p className="small muted" style={{ marginTop: "0.4rem" }}>
                Send user, business-owner, admin-only or bulk operational
                notices with safer targeting.
              </p>
            </Link>

            <Link
              href="/admin/support"
              className="card admin-control-card admin-support-control-card"
            >
              <p className="small muted">Support operations</p>
              <h3>Support inbox and replies</h3>
              <p className="small muted" style={{ marginTop: "0.4rem" }}>
                Review customer, staff and business help requests, then reply
            ...
            <Link href="/admin/directory" className="card admin-control-card">
              <p className="small muted">
                {t("admin.directory.controlKicker", "Marketplace review")}
              </p>
              <h3>
                {t("admin.directory.controlTitle", "Imported place directory")}
              </h3>
              <p className="small muted" style={{ marginTop: "0.4rem" }}>
                {t(
            ...
            >
              <p className="small muted">
                {t("admin.claims.controlKicker", "Ownership review")}
              </p>
              <h3>
                {t("admin.claims.controlTitle", "Directory ownership claims")}
              </h3>
              <p className="small muted" style={{ marginTop: "0.4rem" }}>
                {t(
            ...
            <Link href="/admin/outreach" className="card admin-control-card">
              <p className="small muted">
                {t("admin.outreach.controlKicker", "Owner conversion")}
              </p>
              <h3>
                {t("admin.outreach.controlTitle", "Outreach and follow-ups")}
              </h3>
              <p className="small muted" style={{ marginTop: "0.4rem" }}>
                {t(
          ...
          <div className="admin-dashboard-grid">
            <div className="card">
              <div className="admin-section-header">
                ...
                <Link
                  href="/admin/businesses?attention=attention"
                  className="btn btn-ghost"
                >
              ...
              {attentionBusinesses.length === 0 ? (
                <div className="admin-empty">
                  ...
                  <p className="muted" style={{ marginTop: "0.4rem" }}>
                    No loaded business currently has readiness, billing, trial
                    or pending-booking attention flags.
                  </p>
                </div>
              ) : (
                      ...
                      <div key={business.id} className="admin-business-card">
                        <div>
                          ...
                          >
                            {issues.length > 0
                              ? `Missing: ${issues.join(", ")}`
                              : "Setup fields look complete"}{" "}
                            · {counts.pendingBookings} pending booking
                            {counts.pendingBookings === 1 ? "" : "s"}
                          </p>
            ...
            <div className="card">
              ...
              {trialEndingBusinesses.length === 0 ? (
                <div className="admin-empty">
                  ...
                  <p className="muted" style={{ marginTop: "0.4rem" }}>
                    Trial end dates will appear here once they are set from the
                ...
                <div className="admin-business-list">
                  {trialEndingBusinesses.map(({ business, billing, days }) => (
                    <div
                      key={business.id}
                      className="admin-business-card compact"
                    >
                      <div>
                        <div className="admin-business-title-row">
                          <strong>{business.name}</strong>
                        ...
                        <p
                          ...
                          style={{ marginTop: "0.35rem" }}
                        >
                          Ends {formatDate(billing.trial_end)} ·{" "}
                          {billing.plan_name}
                        </p>
                      </div>
          ...
          <div className="card admin-secondary-section">
            <div className="admin-section-header">
              <div>
          ...
          <div className="card admin-roadmap-card">
            ...
            <h3 style={{ marginTop: "0.25rem" }}>
              This is the internal Mirëbook operator workspace.
            </h3>
            <p className="small muted" style={{ marginTop: "0.5rem" }}>
              Business controls, user lookup, platform notifications, trial
              management and support inbox features should stay here rather than
              inside customer, staff or business-owner workspaces.
            </p>
          </div>
        </div>
      </section>
      ...
      <style jsx>{`
        ...
        @media (max-width: 640px) {
          ...
          .admin-mobile-overview summary::-webkit-details-marker {
            display: none;
          }

          .admin-mobile-overview summary::after {
            content: "+";
            color: var(--accent);
          }

          .admin-mobile-overview[open] summary::after {
            content: "−";
          }
