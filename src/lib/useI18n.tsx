...
import { detectRegionDefaults } from "./regionDefaults";
...
function browserLocale(): Locale {
  ...
  const browserLanguages =
    window.navigator.languages?.map((item) => item.toLowerCase()) || [];
  const languageList = [browserLanguage, ...browserLanguages];

  if (
    languageList.some(
      (language) => language.startsWith("sq") || language.includes("al"),
    )
  ) {
    return "sq";
  }

  if (detectRegionDefaults().locale === "sq") {
    return "sq";
  }
